                ...
                <div class="card-box">
                    <form action="{{ route('stall_locker.store') }}" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="stall_locker_no" id="stall_locker_no" class="form-control" required>
                        </div>
                        ...
                        <button type="submit" class="btn waves-effect waves-light btn-sm" style="background-color: rgb(100, 197, 177); border-color: rgb(100, 197, 177); color: white;">
                            @if($floor->parking_lot && $floor->storage_lot || $floor->bike_lot && $floor->storage_lot || $floor->parking_lot && $floor->bike_lot)
                                Add Stall/Locker
                            @elseif($floor->parking_lot || $floor->bike_lot)
                                Add Stall
                            @elseif($floor->storage_lot)
