...
@section('content')
    @php
        if (!empty($employee_type->id)) {
            $route = route('employee_type.update', $employee_type->id);
            $page_title_prefix = 'Update';
        } else {
            $route = route('employee_type.store');
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <!-- end row -->
            <div class="row">
                <div class="col-md-2"><a href="{{ route('employee_type.index') }}" class="btn btn-success text-capitalize"><i class="fa fa-list"></i> Go To {{ $page_title }} List</a></div>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <select class="form-control" name="status" id="status" required>
                                        <option value="1"
                                            @isset($employee_type->status) {{ $employee_type->status == 1 ? 'selected' : '' }} @endisset>
                                            Active
                                        </option>
                                        <option value="0"
                                            @isset($employee_type->status) {{ $employee_type->status != 1 ? 'selected' : '' }} @endisset>
