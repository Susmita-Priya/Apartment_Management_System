                    ...
                    <div class="card-box">
                        <div style="display:flex; justify-content:space-between;margin:0 14px; ">
                            ...
                            <h4 class="header-title m-b-15 m-t-0">
                                <a href="{{ route('job_location.create') }}" class="btn btn-success">Add data</a>
                            </h4>
                        </div>
                            ...
                            <tbody>
                                @if (!empty($job_locations))
                                    @foreach ($job_locations as $key => $job_location)
                                        <tr>
                                            <td scope="row">{{ ++$key }}</td>
                                            <td>{{ $job_location->name ?? '' }}</td>
                                            <td>{{ $job_location->remarks ?? '' }}</td>
                                            <td class="{{ $job_location->status != 1 ? 'text-danger' : '' }}">
                                                {{ $job_location->status == 1 ? 'Active' : 'Inactive' }}
                                            </td>
                                            <td>
                                                <div class="btn-group dropdown">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                        ...
                                                        {{-- <a class="dropdown-item "
                                                            href="{{ route('job_location.show', ['id' => $job_location->id]) }}"
                                                            type="submit"><i
                                                        ...
                                                        <a class="dropdown-item "
                                                            href="{{ route('job_location.edit', $job_location->id) }}"
                                                            type="submit"><i
                                                                class="mdi mdi-pencil m-r-10 text-muted font-18 vertical-middle"></i>
                                                            Edit
                                                        </a>
                                                        <a class="dropdown-item"
                                                            href="{{ route('job_location.delete', $job_location->id) }}"
