...
@section('content')
    @php
        if (!empty($job_location->id)) {
            $route = route('job_location.update', $job_location->id);
            $page_title_prefix = 'Update';
        } else {
            $route = route('job_location.store');
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <!-- end row -->
            <div class="row">
                <div class="col-md-2"><a href="{{ route('job_location.index') }}" class="btn btn-success text-capitalize"><i class="fa fa-list"></i> Go To {{ $page_title }} List</a></div>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-12">
                                    ...
                                    <input type="text" class="form-control" name="name" id="name"
                                        value="{{ $job_location->name ?? '' }}" required>
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <select class="form-control" name="status" id="status" required>
                                        <option value="1"
                                            @isset($job_location->status) {{ $job_location->status == 1 ? 'selected' : '' }} @endisset>
                                            Active
                                        </option>
                                        <option value="0"
                                            @isset($job_location->status) {{ $job_location->status != 1 ? 'selected' : '' }} @endisset>
