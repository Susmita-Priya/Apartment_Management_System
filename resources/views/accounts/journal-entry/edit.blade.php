...
@section('content')
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-2">
                                    ...
                                    <input type="text" class="form-control" name="journal_code" id="journal_code"
                                        value="{{ $journal_entry->journal_code ?? '' }}" required>
                                ...
                                <div class="form-group col-md-2">
                                    ...
                                    <input type="date" class="form-control" name="entry_date" id="entry_date"
                                        value="{{ $journal_entry->entry_date ?? '' }}" required>
                                    <span class="text-danger">
                                        @error('entry_date')
                                            {{ $message }}
                                        @enderror
                                ...
                                <div class="form-group col-md-8">
                                    ...
                                    <textarea class="form-control" name="remarks" id="remarks" cols="30" rows="10">{{ $journal_entry->remarks ?? '' }}</textarea>
                                    <span class="text-danger">
                                        @error('remarks')
                                            {{ $message }}
                                        @enderror
                                                    ...
                                                    <tr>
                                                                ...
                                                                @if (!empty($accounts))
                                                                    @foreach ($accounts as $key => $account)
                                                                        <option value="{{ $account->id ?? '' }}"
                                                                            @isset($journal_entry_detail->account_id) {{ $journal_entry_detail->account_id == $account->id ? 'selected' : '' }} @endisset>
                                                        ...
                                                        <td>
                                                            <select name="account_group_id[]"
                                                                class="form-control account_group_id"
                                                                index_no="{{ $index_count }}" required>
                                                                <option value="">Slect One</option>
                                                                @if (!empty($account_groups))
                                                                    @foreach ($account_groups as $key => $account_group)
                                                                        <option value="{{ $account_group->id ?? '' }}"
                                                                            ...
                                                                            @isset($journal_entry_detail->account_group_id) {{ $journal_entry_detail->account_group_id == $account_group->id ? 'selected' : '' }} @endisset>
                                                                            {{ $account_group->name ?? '' }}
                                                                        </option>
                                                                    @endforeach
                                                                @endif
                                                        ...
                                                        <td>
                                                            <input type="number" name="debit[]"
                                                                value="{{ $journal_entry_detail->debit ?? null }}"
                                                                class="form-control debit" id="debit_{{ $index_count }}" @if(empty($journal_entry_detail->debit)) readonly @endif>
                                                        </td>
                                                        <td>
                                                            <input type="number" name="credit[]"
                                                                value="{{ $journal_entry_detail->credit ?? null }}"
                                                                class="form-control credit"
                                                                id="credit_{{ $index_count }}" @if(empty($journal_entry_detail->credit)) readonly @endif>
                                                        </td>
                                                        <td><a href="#" class="btn btn-danger remove_button"> <i
    ...
    <input type="hidden" class="index_count" value="{{ $index_count }}">
@endsection
