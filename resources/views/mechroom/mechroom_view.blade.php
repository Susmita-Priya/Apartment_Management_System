...
@section('content')
    @push('title')
        <title>Mechanical Room Details</title>
    @endpush
                    ...
                    <div class="page-title-box">
                        ...
                        <ol class="breadcrumb float-right">
                            ...
                            <li class="breadcrumb-item"><a
                                    href="{{ route('building.show', $mechroom->unit->floor->block->building_id) }}">Building</a>
                            </li>
                            <li class="breadcrumb-item"><a
                                    href="{{ route('block.show', $mechroom->unit->floor->block_id) }}">Block</a></li>
                            <li class="breadcrumb-item"><a
                                    href="{{ route('floor.show', $mechroom->unit->floor_id) }}">Floor</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('unit.show', $mechroom->unit_id) }}">Unit</a></li>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <span class="pull-left m-r-15"><img
                                        src="{{ asset($mechroom->unit->floor->block->building->image) }}" alt=""
                                        class="thumb-lg rounded-circle"></span>
                                <div class="media-body">
                                    <h4 class="m-t-7 font-18">{{ ucfirst(str_replace('_', ' ', $room_type)) }}</h4>
                                  
                                    <p class="text-muted font-15">{{ $mechroom->unit->floor->block->building->name }}
                                        Building</p>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="text-right">
                                    <button type="button" class="btn waves-effect waves-light greenbtn"
                                        ...
                                        onclick="window.location.href='{{ route('mechroom.edit', $mechroom->id) }}'">
                    ...
                    <div class="card-box">
                        ...
                        <div class="panel-body">
                            ...
                            <p class="text-muted font-15"><strong>
                                        Unit ID:
                                </strong> <span class="m-l-15">{{ $mechroom->unit->unit_no }}</span></p>
                            <p class="text-muted font-15"><strong>
                                        Unit Type:
                                </strong> <span class="m-l-15">{{ $mechroom->unit->type }}</span></p>
                     
                            <hr>

                            <p class="text-muted font-15"><strong>Floor No:</strong> <span
                                    class="m-l-15">{{ $mechroom->unit->floor->floor_no }}</span></p>
                            <p class="text-muted font-15"><strong>Floor Name:</strong> <span
                                    class="m-l-15">{{ $mechroom->unit->floor->name }}</span></p>

                            <hr>

                            <p class="text-muted font-15"><strong>Block ID:</strong> <span
                                    class="m-l-15">{{ $mechroom->unit->floor->block->block_id }}</span></p>
                            <p class="text-muted font-15"><strong>Block:</strong> <span
                                    class="m-l-15">{{ $mechroom->unit->floor->block->name }}</span></p>
                            <hr>
                            ...
                            <p class="text-muted font-15"><strong>Building:</strong> <span
                                    class="m-l-15">{{ $mechroom->unit->floor->block->building->name }}</span></p>
                            <p class="text-muted font-15"><strong>Building ID:</strong> <span
                                    class="m-l-15">{{ $mechroom->unit->floor->block->building->building_id }}</span></p>
                            <p class="text-muted font-15"><strong>Building Type:</strong> <span
                                    class="m-l-15">{{ $typeFullForm[$mechroom->unit->floor->block->building->type] ?? 'Other' }}</span>
                            </p>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="text-right m-b-20">
                                {{-- <button type="button" class="btn waves-effect waves-light greenbtn"
                                    onclick="window.location.href='{{ route('asset.create', ['id' => $mechroom->id, 'count' => $roomTypeDetails, 'room_type' => $room_type]) }}'">
                                ...
                                <form id="asset-form" action="{{ route('asset.create') }}" method="GET">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $mechroom->id }}">
                        ...
                        <div class="col-md-12">
                            ...
                            @for ($i = 1; $i <= $roomTypeDetails; $i++)
                                @php
                                    ...
                                    $roomId = $room_type . $i;
                                    $assetId = $mechroom->assets
                                        ? $mechroom->assets->where('room_no', $roomId)->first()->id ?? null
                                        : null;
