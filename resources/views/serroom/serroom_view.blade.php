...
@section('content')
    @push('title')
        <title>Service Room Details</title>
    @endpush
                    ...
                    <div class="page-title-box">
                        ...
                        <ol class="breadcrumb float-right">
                            ...
                            <li class="breadcrumb-item"><a
                                    href="{{ route('building.show', $serroom->unit->floor->block->building_id) }}">Building</a>
                            </li>
                            <li class="breadcrumb-item"><a
                                    href="{{ route('block.show', $serroom->unit->floor->block_id) }}">Block</a></li>
                            <li class="breadcrumb-item"><a
                                    href="{{ route('floor.show', $serroom->unit->floor_id) }}">Floor</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('unit.show', $serroom->unit_id) }}">Unit</a></li>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <span class="pull-left m-r-15"><img
                                        src="{{ asset($serroom->unit->floor->block->building->image) }}" alt=""
                                ...
                                <div class="media-body">
                                    ...
                                    <p class="text-muted font-15">{{ $serroom->unit->floor->block->building->name }}
                                        Building</p>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="text-right">
                                    <button type="button" class="btn waves-effect waves-light greenbtn"
                                        ...
                                        onclick="window.location.href='{{ route('serroom.edit', $serroom->id) }}'">
                    ...
                    <div class="card-box">
                        ...
                        <div class="panel-body">
                            ...
                            <p class="text-muted font-15"><strong>
                                    @if ($serroom->unit->type === 'Residential Suite')
                                        Suite ID:
                                    @else
                                        Unit ID:
                                    @endif
                                </strong> <span class="m-l-15">{{ $serroom->unit->unit_no }}</span></p>
                            <p class="text-muted font-15"><strong>
                                    @if ($serroom->unit->type === 'Residential Suite')
                                        Suite Type:
                                    @else
                                        Unit Type:
                                    @endif
                                </strong> <span class="m-l-15">{{ $serroom->unit->type }}</span></p>
                            ...
                            <p class="text-muted font-15"><strong>Floor No:</strong> <span
                                    class="m-l-15">{{ $serroom->unit->floor->floor_no }}</span></p>
                            <p class="text-muted font-15"><strong>Floor Name:</strong> <span
                                    class="m-l-15">{{ $serroom->unit->floor->name }}</span></p>

                            <hr>

                            <p class="text-muted font-15"><strong>Block ID:</strong> <span
                                    class="m-l-15">{{ $serroom->unit->floor->block->block_id }}</span></p>
                            <p class="text-muted font-15"><strong>Block:</strong> <span
                                    class="m-l-15">{{ $serroom->unit->floor->block->name }}</span></p>
                            <hr>
                            ...
                            <p class="text-muted font-15"><strong>Building:</strong> <span
                                    class="m-l-15">{{ $serroom->unit->floor->block->building->name }}</span></p>
                            <p class="text-muted font-15"><strong>Building ID:</strong> <span
                                    class="m-l-15">{{ $serroom->unit->floor->block->building->building_id }}</span></p>
                            <p class="text-muted font-15"><strong>Building Type:</strong> <span
                                    class="m-l-15">{{ $typeFullForm[$serroom->unit->floor->block->building->type] ?? 'Other' }}</span>
                            </p>
                        ...
                        <div class="col-md-12">
                            ...
                            @for ($i = 1; $i <= $roomTypeDetails; $i++)
                                @php
                                    ...
                                    $roomId = $room_type . $i;
                                    $assetId = $serroom->assets
                                        ? $serroom->assets->where('room_no', $roomId)->first()->id ?? null
                                        : null;
