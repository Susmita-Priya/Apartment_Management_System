...
@section('content')
                    ...
                    <div class="card-box">
                        ...
                        <form action="{{ route('asset.update', $asset->id) }}" enctype="multipart/form-data" method="POST">
                                ...
                                <div class="col-md-5">
                                    ...
                                    <select name="room_id" class="form-control" required>
                                        ...
                                        @php
                                            $roomId = $room_type . $i;
                                        @endphp
                                        <option value="{{ $roomId }}" {{ $roomId == $asset->room_id ? 'selected' : '' }}>{{ $roomId }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                            ...
                            <div id="asset-list" class="mt-3">
                                @foreach ($asset->assets as $index => $existingAsset)
                                    <div class="row asset-item">
                                        <div class="col-md-5">
                                            <label for="asset_name">Asset Name</label>
                                            <input type="text" name="assets[{{ $index }}][name]" class="form-control"
                                                value="{{ $existingAsset->name }}" placeholder="Enter Asset Name" required>
                                        </div>

                                        <div class="col-md-5">
                                            <label for="quantity">Quantity</label>
                                            <input type="number" name="assets[{{ $index }}][quantity]" class="form-control"
                                                value="{{ $existingAsset->quantity }}" placeholder="Enter Quantity" required>
                                        </div>
    ...
    <script>
        let assetIndex = {{ count($asset->assets) }};
        ...
        document.getElementById('add-asset').addEventListener('click', function() {
            ...
            const newAsset = `
                <div class="row asset-item">
                    ...
                    <div class="col-md-2 text-right">
                        <button type="button" class="btn btn-danger remove-extra-field mt-4">Remove</button>
                    </div>
                </div>
            `;
