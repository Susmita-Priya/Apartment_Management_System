...
@section('content')
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            @php
                $subscription_package_text = $subscription_info->subscription_package->name ?? '';
                $package_duration_text = '';
                if (!empty($subscription_info->package_duration)) {
                    $package_duration_text .= $subscription_info->package_duration->value ?? '';
            ...
            <!-- end row -->
            <div class="row">
                <div class="col-md-2"><a href="{{ route('user.index') }}" class="btn btn-success text-capitalize"><i
                            class="fa fa-list"></i>
                        Go To User List</a></div>
                <div class="col-md-6">
                    <form action="{{ route('user_subscription.update', $subscription_info->id) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf

                        {{-- hidden --}}
                        <input type="hidden" name="subscription_info_id" value="{{ $subscription_info->id }}">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="date" class="form-control" name="subcription_date" id="subcription_date"
                                        value="{{ $subscription_info->subcription_date ?? '' }}" required>
                                        ...
                                        @if (!empty($subscription_packages))
                                            @foreach ($subscription_packages as $package)
                                                <option value="{{ $package->id }}"
                                                    @isset($subscription_info->subscription_package_id) {{ $subscription_info->subscription_package_id == $package->id ? 'selected' : '' }} @endisset
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control price" name="package_price" id="package_price"
                                        value="{{ $subscription_info->package_price ?? '' }}" required readonly>
                                    <span class="text-danger">
                                        @error('package_price')
                                            {{ $message }}
                                        @enderror
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control discount_amount" name="discount_amount"
                                        id="discount_amount" value="{{ $subscription_info->discount_amount ?? '' }}"
                                        required readonly>
                                    <span class="text-danger">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control total_amount" name="total_payable_amount"
                                        ...
                                        value="{{ ($subscription_info->package_price ?? 0) - ($subscription_info->discount_amount ?? '') }}"
                                        required readonly>
                                    <span class="text-danger">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control total_paid_amount" name="total_paid_amount"
                                        id="total_paid_amount" value="{{ $subscription_info->total_paid_amount ?? '' }}"
                                        required>
                                    <span class="text-danger">
                                ...
                                <div class="form-group  col-md-6">
                                    ...
                                    <select class="form-control chosen payment_method_type" name="payment_method" required>
                                        <option value="1"
                                            @isset($subscription_info->payment_method) {{ $subscription_info->payment_method == 1 ? 'selected' : '' }} @endisset>
                                            Cash</option>
                                        <option value="2"
                                            @isset($subscription_info->payment_method) {{ $subscription_info->payment_method == 2 ? 'selected' : '' }} @endisset>
                                            Bank</option>
                                        <option value="3"
                                            @isset($subscription_info->payment_method) {{ $subscription_info->payment_method == 3 ? 'selected' : '' }} @endisset>
                                            Mobile Banking</option>
                                    </select>
                                </div>
                                <div class="form-group  col-md-6 online_type" @if (isset($subscription_info->payment_method) && $subscription_info->payment_method == 1) style="display: none;" @endisset>

                                    <div class="mt-2" id="mobile_payment_card">
                                        <div class="form-group">
                                            <label for="payment_details">Payment Details</label>
                                            <textarea name="payment_details" id="payment_details" class="form-control" cols="10" rows="3">{{ $subscription_info->payment_details ?? '' }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
